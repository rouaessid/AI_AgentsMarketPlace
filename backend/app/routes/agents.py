...
async def get_validation(agent_id: str) -> ValidationStatusResponse:
    ...
    if onchain_verdict == "VALID":
        status = "validated"
    elif onchain_verdict in ("INVALID", "DISPUTED"):
        status = "rejected"
    else:
        # Still no on-chain verdict — check if session is stale (backend killed mid-flow)
        status = "in_progress"
...
def _get_val_status(agent_id: str) -> str:
    ...
    if not session:
        return "not_started"
    vtid = session.get("val_task_id")
    if vtid == "FAILED":
        return "failed"
    return "in_progress" if vtid else "not_started"
