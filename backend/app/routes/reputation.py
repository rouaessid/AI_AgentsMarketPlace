...
# ── Debug: recalculate EigenTrust for all agents using current on-chain scores ─
@router.post("/debug/recalculate-eigentrust")
async def recalculate_eigentrust():
    """
    Force EigenTrust recalculation for all agents using their current
    ValidationRegistry.getAgentScore() as seed trust.
    Useful to correct stale 100% scores without waiting for a new validation.
    """
    from app.services.graph_client import get_agent_score
    from app.services.eigentrust_sync import compute_and_write_eigentrust_pipeline

    all_ids = get_all_agent_identities()
    pipeline_agents = [
        a for a in all_ids
        if a.get("current_token_id") and a.get("agent_type", 0) != 1
    ]

    overrides: dict[str, float] = {}
    for agent in pipeline_agents:
        data = get_agent_score(agent["agent_id"])
        score = float(data.get("averageScore", 0)) if data else 0.0
        if score > 0:
            overrides[agent["agent_id"]] = score

    if not overrides:
        return JSONResponse({"status": "no_data", "agents": {}})

    _fire(compute_and_write_eigentrust_pipeline(overrides))
    return JSONResponse({"status": "triggered", "agents": overrides})
...
async def simulate_validation(body: SimulateValidationBody):
    ...
    fresh = get_eigentrust_score(token_id) or 0.0
    _fire(compute_and_write_eigentrust(body.agent_id, fresh))
...
async def feedback_notify(agent_id: str, body: FeedbackNotifyBody):
    ...
    # Feedback is already on-chain — final_score = eigenTrust × user_feedback is computed
    # at read time in GET /{agent_id}. No EigenTrust recomputation needed here.
...
async def get_reputation(agent_id: str):
    ...
    # Count active non-judge agents in network
    all_ids   = get_all_agent_identities()
    n_agents  = len([a for a in all_ids if a.get("current_token_id") and a.get("agent_type", 0) != 1])

    # Pure EigenTrust score (on-chain in ReputationRegistry)
    # N=1 → EigenTrust normalises to 100 (artefact) → use avg_score as pre-trust instead
    eigentrust_100 = (avg_score if n_agents <= 1 else et_raw) if et_raw is not None else avg_score

    starred = raw_signals.get("starred", {})
    user_fb = round(starred.get("average", 0) / 100.0, 4) if starred else None

    # final_score = eigenTrust × user_feedback (both on-chain signals combined)
    # If no feedback yet → final_score = pure eigenTrust (user_feedback factor = 1)
    fb_factor = (starred.get("average", 100) / 100.0) if starred else 1.0
    final_100 = round(eigentrust_100 * fb_factor, 4) if eigentrust_100 else 0.0
