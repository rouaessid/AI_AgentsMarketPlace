...
Decentralized flow:
  1. Platform stores execution trace on IPFS → CID (with challenge_token embedded)
  ...
  5. Returns structured verdict with trajectory_check + challenge_token
"""
...
ORIGINAL TASK:
{trace.get('task_prompt', '(not captured)')[:300]}

FINAL OUTPUT PRODUCED:
{output_str[:5000]}

EXECUTION STATISTICS:
- Total API calls: {len(trajectory)}
- LLM calls: {trace.get('llm_calls', 'N/A')} | Search calls: {trace.get('search_calls', 'N/A')}
- Duration: {trace.get('duration_sec', 'N/A')}s | Tokens: {trace.get('total_tokens', 'N/A')}
