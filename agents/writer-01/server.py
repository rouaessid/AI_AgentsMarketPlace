"""
server.py — WriterBot HTTP server.

POST /api/v1/agents/writer-01/run
  { "task_id": "task-abc", "prompt": "<analysis or content brief>" }
...
async def _lifespan(app: FastAPI):
    ...
    if not groq_key:
        ...
        print(f"[writer-01] ERROR: {_startup_error}", flush=True)
    else:
        try:
            _agent = WriterAgent(groq_api_key=groq_key)
            print("[writer-01] Agent ready", flush=True)
        except Exception as e:
            _startup_error = str(e)
            print(f"[writer-01] Startup failed: {e}", flush=True)
    yield
